...
static const char *topic_base = CONFIG_MQTT_MODULE_TOPIC_BASE;
...
esp_mqtt_client_handle_t mqtt_module_start(esp_event_handler_t cb, void *ctx){
    ...
    if(!nvs_ok){ ESP_ERROR_CHECK(nvs_flash_init()); nvs_ok=true; }
    esp_mqtt_client_config_t cfg = {
        .broker.address.uri = CONFIG_MQTT_MODULE_BROKER_URI,
        .reconnect.timeout_ms = 2000,
    };
    esp_mqtt_client_handle_t client = esp_mqtt_client_init(&cfg);
    if(!client){ ESP_LOGE(TAG, "Fallo al crear cliente MQTT"); return NULL; }
